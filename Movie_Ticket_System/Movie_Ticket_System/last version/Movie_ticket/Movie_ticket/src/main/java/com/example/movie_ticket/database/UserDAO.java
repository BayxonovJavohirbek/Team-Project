...
//    }
    ...
    public static boolean deleteTicket(int ticketId) {
        // First get the ticket details so we can free the seats
        String getTicket =
                "SELECT movie_title, show_time FROM booked_tickets WHERE ticket_id = ?";

        String movieTitle = null;
        String showTime   = null;

        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement ps = conn.prepareStatement(getTicket)) {
            ...
            System.err.println("UserDAO.deleteTicket (fetch): " + e.getMessage());
            return false;
        }

        // Parse seats and show_id from movie_title
        // Format: "The Batman [VIP Hall @ 2026-12-10 19:00] Seats: A1,A2,B3"
        if (movieTitle != null) {
            try {
                // Extract hall name
                int hallStart  = movieTitle.indexOf("[") + 1;
                int atIdx      = movieTitle.indexOf(" @ ");
                int hallEnd    = atIdx;
                String hallName = (hallStart > 0 && hallEnd > hallStart)
                        ? movieTitle.substring(hallStart, hallEnd).trim() : null;

                // Extract seat codes like "A1,B2,C3"
                int seatsIdx = movieTitle.indexOf("Seats: ");
                String seatStr = seatsIdx >= 0
                        ? movieTitle.substring(seatsIdx + 7).trim() : "";

                if (hallName != null && !seatStr.isEmpty() && showTime != null) {
                    // Find the show_id by matching hall_name + show_time
                    String findShow =
                            "SELECT show_id FROM shows " +
                                    "WHERE hall_name = ? AND show_time = ?";

                    int showId = -1;
                    try (Connection conn = DatabaseConnection.getConnection();
                         PreparedStatement ps = conn.prepareStatement(findShow)) {
                        ps.setString(1, hallName);
                        ps.setString(2, showTime);
                        try (ResultSet rs = ps.executeQuery()) {
                            if (rs.next()) showId = rs.getInt("show_id");
                        }
                    }

                    // Free each seat
                    if (showId > 0 && !seatStr.isEmpty()) {
                        String freeSeats =
                                "UPDATE seats SET is_booked = FALSE " +
                                        "WHERE show_id = ? AND row_label = ? AND col_num = ?";

                        try (Connection conn = DatabaseConnection.getConnection();
                             PreparedStatement ps = conn.prepareStatement(freeSeats)) {
                            conn.setAutoCommit(false);

                            for (String seatCode : seatStr.split(",")) {
                                seatCode = seatCode.trim(); // e.g. "A1"
                                if (seatCode.length() < 2) continue;

                                String rowLabel = String.valueOf(seatCode.charAt(0));
                                int    colNum;
                                try {
                                    colNum = Integer.parseInt(
                                            seatCode.substring(1).trim()
                                    );
                                } catch (NumberFormatException ex) {
                                    System.err.println(
                                            "Could not parse seat: " + seatCode);
                                    continue;
                                }
                                ...
                                System.out.println("Freeing seat: " +
                                        rowLabel + colNum +
                                        " for show_id: " + showId);
                            }

                            int[] freed = ps.executeBatch();
                            conn.commit();
                            System.out.println("✅ Freed " + freed.length +
                                    " seats for show_id: " + showId);
                        }
                    }
                }
            } catch (Exception e) {
                System.err.println(
                        "UserDAO.deleteTicket (free seats): " + e.getMessage());
            }
        }
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            System.err.println("UserDAO.deleteTicket (delete): " + e.getMessage());
            return false;
        }
    }
//    public String getStatusFromDB(String login, String password) {
