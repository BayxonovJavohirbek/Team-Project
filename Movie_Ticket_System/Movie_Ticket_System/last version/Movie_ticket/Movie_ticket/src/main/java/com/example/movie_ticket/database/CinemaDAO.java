...
public class CinemaDAO {
    ...
    public void addCinema(String name, Integer cityId) {
        String sql = "INSERT INTO cinemas (name, city_id) VALUES (?, ?)";
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement pstmt = conn.prepareStatement(sql)) {
    ...
    public List<Cinema> getAllCinemas() {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            while (rs.next()) {
                cinemas.add(new Cinema(rs.getInt("cinema_id"), rs.getString("name"), rs.getString("city_id")));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
